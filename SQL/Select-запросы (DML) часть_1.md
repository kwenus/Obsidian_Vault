...
```sql
* Основы:
- выберем все поля из таблицы film (для этого используется * )
SELECT * FROM film;

- выберем столбец title таблицы film (вместо * название столбца)
SELECT title FROM film;

- выберем 2 столбца из таблицы film (нужные столбцы перечисляем через запятую)
SELECT title, release_year FROM film;

* DISTINCT:
- выведем столбец rating из film (выводит уникальные значения стобца)
...
* UPPER
- вывести значения столбца в верхнем регистре
SELECT UPPER(city_name) FROM city

* EXTRACT
- извлечь нужные данные из столбца
